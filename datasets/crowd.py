...
class CustomDataset(Dataset):
    def __init__(
        ...
        """
        Custom dataset. Adjust root directory and image type as needed.
        """
        self.image_paths = [os.path.join(root, img) for img in os.listdir(root) if img.endswith('.jpg') or img.endswith('.png') or img.endswith('.jpeg')]
        self.root = root
        image_files = glob(os.path.join(self.root, "*.*"))
    ...
    def __getitem__(self, idx: int) -> Union[torch.Tensor, Tuple[torch.Tensor, str]]:
        ...
        if self.transforms is not None:
            image, _ = self.transforms(image, torch.tensor([], dtype=torch.float))
        image = self.normalize(image)
...
# Usage example:
# dataset = CustomDataset(root="assets", transforms=None, return_filename=True)
